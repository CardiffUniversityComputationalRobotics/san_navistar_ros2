...
import torch
import numpy as np
from san_navistar_ros2.utils import init
...
class RNNBase(nn.Module):
    ...
    def _forward_gru(self, x, hxs, masks):
        # for acting models, input shape[0] == hidden state shape[0]
...
class SRNN(nn.Module):
    """
    Class representing the SRNN models
    """
    ...
    def forward(self, inputs, rnn_hxs, masks, infer=False):
        ...
        robot_node = reshapeT(inputs["robot_node"], seq_length, nenv)  # [T, env, 1, 7]
        temporal_edges = reshapeT(
            inputs["temporal_edges"], seq_length, nenv
        )  # [T, env, 1, 2]
        spatial_edges = reshapeT(
            inputs["spatial_edges"], seq_length, nenv
        )  # [T, env, 5, 5]
        ...
        masks = reshapeT(masks, seq_length, nenv)  # [T, env, 1]
        ...
        hidden_attn_weighted, _ = self.attn(
            output_temporal, output_spatial
        )  # [T, env_num, 1, 256]
